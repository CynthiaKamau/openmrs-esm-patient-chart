...
export class SubLocationResourceService {
  ...
  private getLocationByUuidFallback(name: string): Observable<SubLocation | undefined> {
    return this.getAllSubLocations().pipe(map((locations) => locations.find((location) => location.name === name)));
  }

  public getLocationByName(name: string): Observable<SubLocation | undefined> {
    const url = this.getUrl();
    return this.http.get<SubLocation>(url).pipe(catchError(() => this.getLocationByUuidFallback(name)));
  }
